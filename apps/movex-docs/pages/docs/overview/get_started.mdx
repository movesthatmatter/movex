...
You can see the code here https://github.com/movesthatmatter/movex-counter-example.
...
In this short tutorial we're going to build a Counter App that will sync in realtime with anybody who loads it out of the box.
...
```ts copy filename="src/movex.config.ts" {1, 5-7}
...
For this tutorial, well keep the UI very basic since it's not the main point.
...
On line 11 we're binding to the counter resource by rid. This means that now we get access to the Resource state by `counter.state` and to the Action Dispatcher by accessing `counter.dispatch()`

In the following lines we're rendering a very simple UI, which displays the Counter State at _line 20_ and adding two Buttons which will be responsible for trigger the "Increment" and "Decrement" actions.
...
{/* Teh counterRid at line 10, simply reads the url */}
